...
;
; PURPOSE:
;   given observed index and errors, and an array of model
...
;
;--------------------------------------------------------------------
FUNCTION index_fit_compress,obs_val,obs_err,model_value,$
                  galid=galid,miniout=miniout,$
                  pars=pars,parnames=parnames,parnorm=parnorm,$
                  nval=nval,magptr=magptr,bandmask=bandmask


;window,1
obs_mag=obs_val
obs_magerr=obs_err
model_mag=model_value
...
for ilvl=0,nlvl-1 do begin
   ;get chi square
   chi2=find_xi(mags,reform(obs_mag[ilvl,*]),reform(obs_magerr[ilvl,*]),norm=norm,mask=bandmask)
   bchi=min(chi2,imin)
