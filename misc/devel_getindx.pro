...
;  !---------------------------------------------------------------!
  ...
  FOR j=0,nindsps-1 DO BEGIN
     ...
     cberr=sqrt(intind(lambda,spec_err^2,indexdef[2,j],indexdef[3,j]))
     cb = cb / (indexdef[3,j]-indexdef[2,j])     
     cberr=cberr/(indexdef[3,j]-indexdef[2,j])   
     lb = (indexdef[2,j]+indexdef[3,j])/2.

     IF (indexdef[6,j] NE 4) THEN BEGIN
        ;;!red continuum 
        cr = intind(lambda,spec,indexdef[4,j],indexdef[5,j])
        crerr = sqrt(intind(lambda,spec^2,indexdef[4,j],indexdef[5,j]))
        cr = cr / (indexdef[5,j]-indexdef[4,j])
        cr = crerr / (indexdef[5,j]-indexdef[4,j])
        ...
        cbarr_err=(fltarr(nw)+1)#cberr
        fact=(fltarr(nw)+1)#((cr-cb)/(lr-lb))
        fc=(larr-lb)*fact+cbarr
        ;;vectorised
        intfifc=intind(lambda,spec/fc,indexdef[0,j],indexdef[1,j])
        intfifc_err=intind(lambda,(spec_err/fc)^2+(fc_err*spec/fc^2)^2,indexdef[0,j],indexdef[1,j])
