...
for iz=0,n_elements(z)-1 do begin
   ...
   scriptname='cb07_scripts/script_cb07allimf_grid_onedust_exp_z'+$
   ...
   for im=0,1 do begin
      for it=0,n_elements(tau_sf)-1 do begin
         ;;if negative tau_sf use a different script (rising SFH)
         ;;loop over dust
         for imu=0,n_elements(mu_dust)-1 do begin
            for id=0,n_elements(tau_v)-1 do begin

               outname='cb07'+imf[im]+'_exp'+$
               ...
               if file_test(rundir+zdir+outname+'.ised') EQ 0 then begin 
                  ...
                  printf,1,'cb2007_hr_stelib_'+zname[iz]+'_'+imf[im]+'_ssp.ised'
                  printf,1,'Y'  ;add dust
   ...
   printf,1,'mv cb07*_exp*_z'+strcompress(string(round(z[iz]*100),format='(I3.3)'),/remove_all)+'* '+$
          rundir+zdir
   close,1
