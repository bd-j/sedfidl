...
;                       precision 
;       B.D.J, 2006 Oct. 12th  - changed offs to 4 to accomodate CSP ised
;                                files.  Also added EXTRANAME
;                                parameter to read extra bc03 files
;                                from nonstandard filenames
;       bdj, 2009 Feb - added endian keyword to allow for ised files
;                       created with different architectures
;-
...
headlines = where(stregex(temp,'#',/boolean) eq 1B,nheadlines)
if (nheadlines eq 0L) then $
  message, 'There is a problem with '+extrapath+extrafile+'.'

agerow = where(stregex(temp,'log-age',/fold,/boolean) eq 1B,nagerow)
...
function k_im_read_bc03_v3, isedfile=isedfile, isedpath=isedpath, $
                         ...
                         bc03_extras=bc03_extras,extraname=extraname,$
                         endian=endian
...
; read the extra parameters for this SSP
    ...
    colorfiles = base+string(lindgen(4)+1,format='(I1)')+'color'
...
; remove repeated structure tag names before appending the data             
                ...
                bc03_extras = struct_combine(bc03_extras,extras1)
